...
template<typename T>
struct tb_traits<std::complex<T>>
{
    typedef T BASE_T;
};
...
#ifdef HLS_COMPLEX
template<typename T>
struct tb_traits<hls::x_complex<T>>
{
    typedef T BASE_T;
};
#endif
...
struct NoTranspose
{
    ...
    static InputType GetElement(const InputType A[RowsA][ColsA], unsigned Row,
                                unsigned Col)
    {
...
struct ConjugateTranspose
{
                                ...
                                unsigned Col)
    {
        ...
        // o For non std::complex types this function will return an unaltered
        // value.
...
InputType GetMatrixElement(const InputType A[RowsA][ColsA], unsigned Row,
                           unsigned Col)
{
    // Need to help the compiler identify that the GetElement member function is
    // a template
    return TransposeForm::template GetElement<RowsA, ColsA, InputType>(A, Row,
                                                                       Col);
}
...
// Complex data
// ----------------
// This is used by complex data of any type.  A printer is called to print the
// real and imaginary parts, so this just handles the formating of x+jy
template<typename T>
struct xil_printer<std::complex<T>>
{
    static std::string to_s(std::complex<T> x, unsigned prec = 10)
    {
        ...
        // Casting to "T" avoids "error: operands to ?: have different types."
        // when using ap_fixed.
        T imag = neg_imag ? (T)-x.imag() : (T)x.imag();

        ss << printer::to_s(x.real(), prec) << (neg_imag ? " - j*" : " + j*")
           << printer::to_s(imag, prec);
        return ss.str();
    }
};

template<unsigned ROWS, unsigned COLS, typename T, class TransposeForm>
void print_matrix(T a[ROWS][COLS], std::string prefix = "", unsigned prec = 10,
                  unsigned matlab_format = 0)
{
    typedef xil_printer<T> printer;
...
void MatMult(T A[rowsA][colsA], T B[colsA][colsB], T C[rowsA][colsB])
{
    T tmp[rowsA][colsB];
