// -------------------------------------------------------------------------------
// --                   LICENSE NOTICE
// -------------------------------------------------------------------------------
// --
// -- Copyright 2006-2011,2013-2014 Free Software Foundation, Inc.
// --
// -- This file is part of GNU Radio
// --
// -- SPDX-License-Identifier: GPL-3.0-or-later
// --
// --
// -------------------------------------------------------------------------------
// --
// -- Company:     BlackLynx, Inc.
// --
// -- File Name: mgs_inverse_complex.cpp
// -- Description: Streamed/batched matrix inversion implemented using QR decomposition
// --              (based on modified Gram-Schmidt algorithm)
// --
// -- Dependencies: "../include/mgs_inverse_complex.h"
// -- Dependencies: "../include/constants.h"
// --
// -------------------------------------------------------------------------------
// --                                DETAILS
// -------------------------------------------------------------------------------
// --
// -- TBD
// --
// -------------------------------------------------------------------------------
// --                                HISTORY
// -------------------------------------------------------------------------------
// --
// -------------------------------------------------------------------------------
#include "constants.h"
#include "mgs_inverse_complex.h"
#include "hls_math.h"

// The top-level function to synthesize
void mgs_inverse_complex(hls::stream<COMPLEX_MGS_FP_WIDE_T> &A_vec,
                         hls::stream<COMPLEX_MGS_FP_WIDE_T> &InverseA_vec)
{
  #pragma HLS INTERFACE ap_ctrl_none port=return
  //#pragma HLS INTERFACE axis register port=A_vec
  //#pragma HLS INTERFACE axis register port=InverseA_vec
  #pragma HLS INTERFACE axis port=A_vec
  #pragma HLS INTERFACE axis port=InverseA_vec

  const int NUM_ITERATIONS = (INVERSE_BATCH_SIZE+INVERSE_PARALLELIZATION_FACTOR-1)/INVERSE_PARALLELIZATION_FACTOR;

  MATRIX_IN_T  a_vec_i        [INVERSE_PARALLELIZATION_FACTOR][NUM_ITERATIONS][ROWS_COLS_A][ROWS_COLS_A];
  MATRIX_OUT_T inverse_a_vec_i[INVERSE_PARALLELIZATION_FACTOR][NUM_ITERATIONS][ROWS_COLS_A][ROWS_COLS_A];

  #pragma HLS ARRAY_PARTITION variable=a_vec_i cyclic factor=INVERSE_PARALLELIZATION_FACTOR dim=1
  #pragma HLS ARRAY_PARTITION variable=inverse_a_vec_i cyclic factor=INVERSE_PARALLELIZATION_FACTOR dim=1

  // Copy input data to local memory
  for (int x=0; x<INVERSE_BATCH_SIZE; x++)
  {
    for (int r=0;r<ROWS_COLS_A;r++)
    {
      for (int c=0;c<ROWS_COLS_A;c++)
      {
        COMPLEX_MGS_FP_WIDE_T temp;
        #pragma HLS PIPELINE
        A_vec >> temp;
        a_vec_i[x/NUM_ITERATIONS][x%NUM_ITERATIONS][r][c] = temp.data.data[0];
      }
    }
  }

  for (int x=0; x<INVERSE_PARALLELIZATION_FACTOR; x++) {
    #pragma HLS UNROLL factor=INVERSE_PARALLELIZATION_FACTOR
    mgs_qr_inverse_complex_stream_vectorized<NUM_ITERATIONS, ROWS_COLS_A>(a_vec_i[x], inverse_a_vec_i[x]);
  }

  // Copy local memory contents to output
  for (int x=0, count=0; x<INVERSE_BATCH_SIZE; x++)
  {
    for (int r=0;r<ROWS_COLS_A;r++)
    {
      for (int c=0;c<ROWS_COLS_A;c++, count++)
      {
        COMPLEX_MGS_FP_WIDE_T temp;
        #pragma HLS PIPELINE
        temp.last = (count == ((INVERSE_BATCH_SIZE*ROWS_COLS_A*ROWS_COLS_A)-1)) ? 1 : 0;
        temp.keep = -1;
        temp.data.data[0] = inverse_a_vec_i[x/NUM_ITERATIONS][x%NUM_ITERATIONS][r][c];
        InverseA_vec << temp;
      }
    }
  }
}

template <int NUM_ITERATIONS, int ROWS_COLS_A>
void mgs_qr_inverse_complex_stream_vectorized(MATRIX_IN_T A_vec[NUM_ITERATIONS][ROWS_COLS_A][ROWS_COLS_A],
                                              MATRIX_OUT_T InverseA_vec[NUM_ITERATIONS][ROWS_COLS_A][ROWS_COLS_A]){

  #pragma HLS DATAFLOW

  MATRIX_IN_T  Q_vec           [NUM_ITERATIONS][ROWS_COLS_A][ROWS_COLS_A];
  MATRIX_IN_T  R_vec           [NUM_ITERATIONS][ROWS_COLS_A][ROWS_COLS_A];

  // Call wrapped QR Decomposition - Q is being output such that Q*R = A
  // Gram-Schmidt Reduced Decomposition
  mgs_qrd_complex_vectorized<NUM_ITERATIONS, ROWS_COLS_A>(A_vec,Q_vec,R_vec);

  // Convert final A^-1 from Q and R.
  QR_to_InverseA_vectorized<NUM_ITERATIONS, ROWS_COLS_A>(Q_vec, R_vec, InverseA_vec);
}

template <int NUM_ITERATIONS, int ROWS_COLS_A>
void mgs_qrd_complex_vectorized(MATRIX_IN_T  a_matrix_vec[NUM_ITERATIONS][ROWS_COLS_A][ROWS_COLS_A],
                                MATRIX_OUT_T q_matrix_vec[NUM_ITERATIONS][ROWS_COLS_A][ROWS_COLS_A],
                                MATRIX_OUT_T r_matrix_vec[NUM_ITERATIONS][ROWS_COLS_A][ROWS_COLS_A]) {

  for (int x=0; x<NUM_ITERATIONS; x++) {
    mgs_qrd_complex(a_matrix_vec[x], q_matrix_vec[x], r_matrix_vec[x]);
  }
}

template <int ROWS_COLS_A>
void mgs_qrd_complex(MATRIX_IN_T  a_matrix[ROWS_COLS_A][ROWS_COLS_A],
                     MATRIX_OUT_T q_matrix[ROWS_COLS_A][ROWS_COLS_A],
                     MATRIX_OUT_T r_matrix[ROWS_COLS_A][ROWS_COLS_A]) {

/* Modified Gram Schmidt QR Decomposition Algorithm
   QRD(input matrixA, output Q, output R)
1. copy matrixA into T

   for (i= 1:n)                 // main loop (over columns)
2.   l2_norm = ||Ti||           // l2-norm(magnitude) of ith column vector
3.   Rii = l2_norm              // Assign alpha to R diagonal element
4.   Qi = Ti / l2_norm          // Compute the ith Q column: divide ith T column vector by alpha
     for (j=i+1:n)              // inner loop
5.     dotProduct = Qi.Tj       // Compute the dot product between the ith column of Q and the jth column of T
6.     Rij= dotProduct          // Assign the the ith row of R off diagonal elements the computed dot product
7.     Tj= Ti - dotProduct x Qi // Update the jth column of T: Current jth column of T minus the dot product times the ith column of Q
      end
    end
  end //QRD

*/

  MATRIX_IN_T t_matrix[ROWS_COLS_A][ROWS_COLS_A]; // dynamically allocated
  MATRIX_OUT_T q_matrix_temp[ROWS_COLS_A][ROWS_COLS_A];

  for (int r=0;r<ROWS_COLS_A;r++) {
    for (int c=0;c<ROWS_COLS_A;c++) {
      #pragma HLS PIPELINE
      t_matrix[r][c] = a_matrix[r][c];
    }
  }

  for (int i = 0; i < ROWS_COLS_A; i++) {

     // line 2, 3 of algorithm: calculated alpha (t_magnitude)
     float t_magnitude = magnitudeColumn<ROWS_COLS_A>(t_matrix, i);
     float t_magnitude_reciprocal = 1.0f / t_magnitude;
     r_matrix[i][i].real(t_magnitude);
     r_matrix[i][i].imag(0.0f);

     // line 4 of algorithm
     // generate the normalized ith column of the q_matrix matrix
     for (int mRow = 0; mRow < ROWS_COLS_A; mRow++) {
       hls::x_complex<float> temp = t_matrix[mRow][i] * t_magnitude_reciprocal;
       q_matrix_temp[mRow][i] = temp;
       q_matrix[mRow][i] = temp;
     }

     for (int j = 0; j < ROWS_COLS_A; j++) {
       if (j > i) {
         // line 5, 6 of algorithm
         hls::x_complex<float> dotProduct = 0.0f;
         for (int mRow = 0; mRow < ROWS_COLS_A; mRow++) {
           dotProduct += hls::x_conj(q_matrix_temp[mRow][i]) * t_matrix[mRow][j];
         }
         r_matrix[i][j] = dotProduct;
         r_matrix[j][i] = 0.0f;

         // line 7 of algorithm
         // generate the next column of the T-matrix.
         for (int mRow = 0; mRow < ROWS_COLS_A; mRow++) {
           #pragma HLS PIPELINE
           hls::x_complex<float> tj = t_matrix[mRow][j];
           hls::x_complex<float> qi = q_matrix_temp[mRow][i];
           t_matrix[mRow][j] = tj - (dotProduct * qi);
         }
       }
     }
  }
}

template <int ROWS_COLS_A>
float magnitudeColumn(MATRIX_IN_T matrixData[ROWS_COLS_A][ROWS_COLS_A], int targetColumn) {

  float sum = 0;
  for (int row = 0; row < ROWS_COLS_A; row++) {
    hls::x_complex<float> val = matrixData[row][targetColumn];
    float val_real = val.real();
    float val_imag = val.imag();
    sum += (val_real * val_real) + (val_imag * val_imag);
  }
  return hls::sqrtf(sum);
}

template <int NUM_ITERATIONS, int ROWS_COLS_A>
void QR_to_InverseA_vectorized(MATRIX_IN_T Q_vec[NUM_ITERATIONS][ROWS_COLS_A][ROWS_COLS_A],
                               MATRIX_IN_T R_vec[NUM_ITERATIONS][ROWS_COLS_A][ROWS_COLS_A],
                               MATRIX_OUT_T InverseA_vec[NUM_ITERATIONS][ROWS_COLS_A][ROWS_COLS_A]){

  for (int x=0; x<NUM_ITERATIONS; x++) {

    MATRIX_IN_T InverseR [ROWS_COLS_A][ROWS_COLS_A];

    // Invert R matrix (upper triangular)
    invert_R<ROWS_COLS_A>(R_vec[x], InverseR);

    // Compute Ainv = Rinv * Qtranspose
    matrix_multiply_RinvQt<ROWS_COLS_A>(InverseR, Q_vec[x], InverseA_vec[x]);

  }
}

template <int ROWS_COLS_A>
void invert_R(MATRIX_IN_T  R[ROWS_COLS_A][ROWS_COLS_A],
              MATRIX_OUT_T InverseR[ROWS_COLS_A][ROWS_COLS_A]) {

  hls::x_complex<float> R_reciprocal_diag[ROWS_COLS_A];

  for (int m=0; m<ROWS_COLS_A; m++) {
    #pragma HLS PIPELINE
    hls::x_complex<float> one = 1.0f;
    R_reciprocal_diag[m] = one / R[m][m];
  }

  for (int m=0; m<ROWS_COLS_A; m++) {
    #pragma HLS PIPELINE II=1
    //#pragma HLS UNROLL
    for (int n=0; n<ROWS_COLS_A; n++) {
      //#pragma HLS PIPELINE II=1
      //#pragma HLS UNROLL
      hls::x_complex<float> S;
      if (n >= m) {
        S = (m == n) ? 1.0f : 0.0f;

        for (int p=0; p<ROWS_COLS_A; p++) {
           if ((p>=m) && (p<n))
           S -= InverseR[m][p] * R[p][n];
        }
        InverseR[m][n] = S * R_reciprocal_diag[n];
      }
    }
  }
}


template <int ROWS_COLS_A>
void matrix_multiply_RinvQt(MATRIX_IN_T  InverseR[ROWS_COLS_A][ROWS_COLS_A],
                            MATRIX_IN_T  Q[ROWS_COLS_A][ROWS_COLS_A],
                            MATRIX_OUT_T InverseA[ROWS_COLS_A][ROWS_COLS_A]) {

  for (int c = 0; c < ROWS_COLS_A; c++) {
    for (int d = 0; d < ROWS_COLS_A; d++) {
      #pragma HLS PIPELINE
      hls::x_complex<float> sum = 0.0f;
      for (int k = 0; k < ROWS_COLS_A; k++) {
        if (k >= d)
          sum += InverseR[d][k] * hls::x_conj(Q[c][k]);
      }
      InverseA[d][c] = sum;
    }
  }
}

